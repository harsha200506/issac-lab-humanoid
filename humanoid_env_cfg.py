...
class RewardsCfg:
    ...
    progress = RewTerm(func=mdp.progress_reward, weight=0.3, params={"target_pos": (1000.0, 0.0, 0.0)})
    # (2) Stay alive bonus
    alive = RewTerm(func=mdp.is_alive, weight=5.0)
    # (3) Reward for non-upright posture
    upright = RewTerm(func=mdp.upright_posture_bonus, weight=2.0, params={"threshold": 0.93})
    ...
    # (8) Penalty for arms deviating from natural resting position
    joint_deviation = RewTerm(
        func=mdp.joint_deviation_l1,
        weight=-0.2,
        params={"asset_cfg": SceneEntityCfg("robot", joint_names=[".*_upper_arm.*", ".*_lower_arm.*"])},
    )
...
class TerminationsCfg:
    ...
    torso_height = DoneTerm(func=mdp.root_height_below_minimum, params={"minimum_height": 0.9})
